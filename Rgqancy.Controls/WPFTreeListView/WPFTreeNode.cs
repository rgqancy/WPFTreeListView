...
{
	public sealed class WPFTreeNode : INotifyPropertyChanged
	{
		#region NodeCollection
		private class NodeCollection : Collection<WPFTreeNode>
		{
			private WPFTreeNode _owner;

			public NodeCollection(WPFTreeNode owner)
			{
				_owner = owner;
			}
			...
			protected override void InsertItem(int index, WPFTreeNode item)
			{
				if (item == null)
			...
			protected override void SetItem(int index, WPFTreeNode item)
			{
				if (item == null)
					throw new ArgumentNullException("item");
				RemoveAt(index);
		...
		private WPFTreeListView _tree;
		public WPFTreeListView Tree
		{
			get { return _tree; }
		}
		...
		private WPFTreeNode _parent;
		public WPFTreeNode Parent
		{
			get { return _parent; }
		}
		...
		public WPFTreeNode PreviousNode
		{
			get
			{
				if (_parent != null)
		...
		internal WPFTreeNode BottomNode
		{
			get
			{
				WPFTreeNode parent = this.Parent;
				if (parent != null)
				{
		...
		internal WPFTreeNode NextVisibleNode
		{
			get
			{
		...
		public IEnumerable<WPFTreeNode> AllVisibleChildren
		{
			get
			{
				int level = this.Level;
				WPFTreeNode node = this;
				while (true)
				{
		...
		private Collection<WPFTreeNode> _children;
		internal Collection<WPFTreeNode> Children
		{
			get { return _children; }
		}

		private ReadOnlyCollection<WPFTreeNode> _nodes;
		...
		internal WPFTreeNode(WPFTreeListView tree, object tag)
		{
			if (tree == null)
			...
			_nodes = new ReadOnlyCollection<WPFTreeNode>(_children);
			_tag = tag;
		}
